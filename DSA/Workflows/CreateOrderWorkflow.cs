...
namespace Dsa.Workflows
{
	public class CreateOrderWorkflow
	{
		...
		#region Create Order From Create Quote Page  -- using updated method from PCFOrderDetailsPage.
	...
	/*	public static OrderDetailsPage CreateOrderFromCreateQuotePage(IWebDriver driver, string paymentMethod)
		...
		public static PaymentsPage CreateOrderTillPaymentPage(IWebDriver driver, string sendTo = "com_qe@dell.com", string attachmentType = "PDF Attachment", bool modAck = true)
		{
			//  new QuoteDetailsPage(driver).WaitForQuoteValidationToComplete();
			new QuoteDetailsPage(driver).CreateOrder();
			new ExportCompliancePage(driver).WillNotExportOutsideUs();
			if (modAck)
				new EmailConfirmationPage(driver).ProcessOrderAcknowledgement(attachmentType, sendTo);
			else
			{
				new EmailConfirmationPage(driver).BtnEmailConfirmationNext().Click(driver);
			}
			return new PaymentsPage(driver);

		}

		public static PaymentsPage PCFCreateOrderTillPaymentPage(IWebDriver driver, string sendTo = "com_qe@dell.com", string attachmentType = "PDF Attachment", bool modAck = true)
		{
			//  new QuoteDetailsPage(driver).WaitForQuoteValidationToComplete();
			new PCFQuoteSummaryPage(driver).CreateOrder();			
			new PCFComplianceNotificationsPage(driver).BtnContiueCheckOut.Click(driver);		
			return new PaymentsPage(driver);
		}
		...
		public static void GetToSubscriptionBillingFromQuoteDetails(IWebDriver driver)
		{
			new ExportCompliancePage(driver).WillNotExportOutsideUs();
			new EmailConfirmationPage(driver).BtnEmailConfirmationNext().Click(driver);
		}
		...
		public static PaymentsPage EnterOrderType(IWebDriver driver, int OrderMethodIndex, OrderType orderType, string PONumber)
		{
			if (OrderMethodIndex > 1)
				new PaymentsPage(driver).DdlOrderType.Click(driver);
			switch (orderType)
			{
				case OrderType.GiveawaysMarketing:
					//quoteSaveAsOrderPayment_poNumber
					// new PaymentsPage(driver).EnterPONumber(PoNumber);
					new PaymentsPage(driver).OrderTypeMethod(OrderMethodIndex).PickDropdownByText(driver, "Giveaways - Marketing");
					new PaymentsPage(driver).EnterPurchaseOrderNumber(PONumber);
					break;

				case OrderType.DELLCapitalEquipNew:
					//quoteSaveAsOrderPayment_poNumber
					// new PaymentsPage(driver).EnterPONumber(PoNumber);
					new PaymentsPage(driver).OrderTypeMethod(OrderMethodIndex).PickDropdownByText(driver, "DELL Capital Equip New");
					new PaymentsPage(driver).EnterPurchaseOrderNumber(PONumber);
					break;

				case OrderType.EMCCapitalEquipNew:
					//quoteSaveAsOrderPayment_poNumber
					// new PaymentsPage(driver).EnterPONumber(PoNumber);
					new PaymentsPage(driver).OrderTypeMethod(OrderMethodIndex).PickDropdownByText(driver, "EMC Capital Equip New");
					new PaymentsPage(driver).EnterPurchaseOrderNumber(PONumber);
					break;
			}
			return new PaymentsPage(driver);
		...
		{
			driver.WaitForElementDisplay(By.Id("quoteSaveAsOrderPayment_addPayment"), TimeSpan.FromSeconds(10));
		...
		public static void ProcessIndividualPaymentMethod(IWebDriver driver, string paymentMethod, int index, double? amount = null)
		{
			...
			if (index > 1)
			{
				new PaymentsPage(driver).ApplyRemainderAmount(index, true);
			}
		}
		...
		public static PaymentsPage CompleteDFSInformation(IWebDriver driver, string lastName = "", string pin = "")
		{
			...
			if (!string.IsNullOrEmpty(lastName) && !string.IsNullOrEmpty(pin))
				new PaymentsPage(driver).CompleteDfsInformation().EnterDfsInformation<PaymentsPage>(driver, lastName, pin);
			return new PaymentsPage(driver);
		}
		...
		public static OrderReviewPage EnterPaymentInformationWithQuickLease(IWebDriver driver, string processMethod)
		{
			...
			paymentPage = dfsPaymentPage.EnterQuickLeaseData(driver, processMethod);
			paymentPage.PaymentMethodNext();
			return new OrderReviewPage(driver);
		}

		public static OrderReviewPage EnterPaymentInformationWithMLA(IWebDriver driver, string processMethod)
		{
			var paymentPage = new PaymentsPage(driver);
			paymentPage.PaymentMethodNext();
			if (paymentPage.IsAddPaymentReminder())
		...
		#region Funder
		public static void ValidateFunderDropdown(IWebDriver driver)
		{
			var paymentPage = new PaymentsPage(driver);
		}
		#endregion
		...
		public static bool OrderProcessingStatusCancelledAfterCancel(IWebDriver driver)
		{
			...
			while (attempts != 0)
			{
				var processingStatus = orderDetails.OrderProcessiongStatus.GetText(driver);
