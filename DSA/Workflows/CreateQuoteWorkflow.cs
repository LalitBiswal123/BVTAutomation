    ...
    public static class CreateQuoteWorkflow
    {
            ...
            string contractCode = "", string quoteName = "", bool saveQuote = true, string endUserCustomer = "")
        {
            ...
            if (!string.IsNullOrEmpty(quoteName))
                driver.WaitForElementDisplay(crQuotePage.TxtQuoteName, TimeSpan.FromSeconds(30));
          ...
          string contractCode1 = "", string contractCode2 = "", string quoteName = "", bool saveQuote = true)
        {
            ...
            //new CreateQuotePage(driver).GetRecommendedTabAtIndex(0).Click();
            new CreateQuotePage(driver).ClickAddShippingGroup();
        ...
        {
            CustomerSearchWorkflow.SearchCustomerByCustomerNumber(driver, customerNumber);
            new CustomerDetailsPage(driver).CreateQuote();
        ...
        #region Create Quote from Customer Search
        public static void CreateQuoteFromCustomerSearch_AddItemsFromTop(IWebDriver driver, string customerNumber, Dictionary<string, ProductSearchType> products,
            string contractCode = "", string quoteName = "", bool saveQuote = true, bool SnPasConfig = false, string enduserNo = "")
        {
            CustomerSearchWorkflow.SearchCustomerByCustomerNumber(driver, customerNumber).CreateQuote();
            if (!string.IsNullOrEmpty(quoteName))
                new CreateQuotePage(driver).TxtQuoteName.SetText(driver, quoteName);
            AddMoreItemsToQuoteFromTop(driver, products, SnPasConfig);
            if (!string.IsNullOrEmpty(contractCode))
                new CreateQuotePage(driver).ApplyContractCodeToAllItems(contractCode);
            //new CreateQuotePage(driver).ClickICodeOverride(); team need to add this action in their script
            if (!string.IsNullOrEmpty(enduserNo))
        ...
        public static void AddMoreItemsToQuote(IWebDriver driver, Dictionary<string, ProductSearchType> products,bool SnPasConfig=false, bool isCustomerPatched = true)
        {
            ...
            new CreateQuotePage(driver).AddItemsFromSearch().BtnViewQuote.WaitForElementDisplayed(TimeSpan.FromMinutes(1));
            new ProductSearchPage(driver).ConfigurableProductTree.WaitForElementDisplayed(TimeSpan.FromSeconds(10));
            ...
            foreach (KeyValuePair<string, ProductSearchType> item in products)
            {
                ...
                if (new ProductSearchPage(driver).CheckBox.IsElementDisplayed(driver))
                {
                    new ProductSearchPage(driver).CheckBox.Click();
                    new ProductSearchPage(driver).CartOk.Click();
                    new ProductSearchPage(driver).BtnSuccess.Click();
                 }
        ...
        //Add items to search from top link
        public static void AddMoreItemsToQuoteFromTop(IWebDriver driver, Dictionary<string, ProductSearchType> products, bool SnPasConfig = false, bool isCustomerPatched = true)
        {
            new CreateQuotePage(driver).AddItemsFromSearchTopButton().BtnViewQuote.WaitForElementDisplayed(TimeSpan.FromMinutes(1));
            ...
            SplitLineValidation typeSplitLineValidation, int groupIndex = 1)
        {
            ...
            quoteData.Discount = double.Parse(new CreateQuotePage(driver).GetItemLevelDiscount( index,groupIndex));
        ...
        {
            var customerDetailsPage = CustomerSearchWorkflow.SearchCustomerByCustomerNumber(driver, customerNumber);
            try
            {
        ...
        #region Add Quote and Patch A Customer

        public static void CreateQuotePatchCustomer(IWebDriver driver, Catalogs catalog, string customerNumber)
        {
            var createQuote = new CreateQuotePage(driver);
            createQuote.SelectCatalog(catalog);
            createQuote.TxtCustomerNumber.SetText(driver, customerNumber);
            createQuote.ValidateCustomerNumberPatch();
        }
        #endregion 
        ...
        public static void MoveItemToShippingGroup(IWebDriver driver, int shippingGroupIndex, int productIndex, int destinationShippingGroupNumber)
        {
            ...
            new CreateQuotePage(driver).DropDwnShippingGroup.PickDropdownByIndex(driver, destinationShippingGroupNumber);
            new CreateQuotePage(driver).BtnMove.Click(driver);
        }

        public static void MoveLinkItemLevel(IWebDriver driver, int shippingGroupIndex, int productIndex)
        {
            new CreateQuotePage(driver).MoveItem(shippingGroupIndex, productIndex).Click(driver);
        }

        public static bool MoveLinkVerifyAddNewShippingGroup(IWebDriver driver, int shippingGroupIndex, int productIndex)
        {
            new CreateQuotePage(driver).MoveItem(shippingGroupIndex, productIndex).Click(driver);
            new CreateQuotePage(driver).AddNewShippingGroup().Click();

            try
            {
                new CreateQuotePage(driver).DropDwnShippingGroup.PickDropdownByText(driver, " New Shipping Group ");
                return true;
            }
            catch
            {
        ...
        public static bool MoveLinkVerifyAddNewShippingGroupGrayed(IWebDriver driver, int shippingGroupIndex, int productIndex)
        {
            new CreateQuotePage(driver).MoveItem(shippingGroupIndex, productIndex).Click(driver);
            new CreateQuotePage(driver).AddNewShippingGroup().Click();
            ...
            {
                new CreateQuotePage(driver).DropDwnShippingGroup.PickDropdownByText(driver, " New Shipping Group ");
                new CreateQuotePage(driver).AddNewShippingGroup().GetAttribute("disabled");
                return true;
            }
            catch
            {
        ...
        public static void CopyShippingGroup(IWebDriver driver,int shippingGroupIndex, int numberOfCopies, string selectItems)
        {
            CreateQuotePage createQuotePage = new CreateQuotePage(driver);
            createQuotePage.CopyGroup(shippingGroupIndex).Click();
        ...
        public static void ValidateOrderAndSku(IWebDriver driver, Dictionary<string, ProductSearchType> products, int shippingIndex = 1)
        {
            ...
            { 
                // Todo: Revisit this commented code. For now, reverting this MR https://gitlab.dell.com/qe/qeo-commerce-experiences-and-platform/qeo-dce-seller/dsa/dsa-test-automation/-/merge_requests/303 as it is breaking the DVTs
                //if (new CreateQuotePage(driver).CheckItemIsExpandOrCollaspeMode("Manual", shippingIndex, itemIndex))
                //{
                //    new CreateQuotePage(driver).ExpandSolutionModule(itemIndex, shippingIndex);
                //}
        ...
        public static void CreateQuoteFromMenu(IWebDriver testWebDriver, string customerNumber, Dictionary<string, ProductSearchType> products, string v1, string quoteName, bool v2)
        {
        ...
        #region RSID Quote Creation
        public static void ValidateRSIDQuoteFromMenu(IWebDriver driver, Catalogs catalog, string customerNumber, Dictionary<string, ProductSearchType> products,
           string contractCode = "", string quoteName = "", bool saveQuote = true, string endUserCustomer = "", string CFIProject = "", string CFIComment = "")
        {
            ...
            if (!string.IsNullOrEmpty(endUserCustomer))
                crQuotePage.TxtEndUserCustomerNumber.SetText(driver, endUserCustomer);
            if (!string.IsNullOrEmpty(CFIProject))
            {
                new CreateQuotePage(driver).GetConfigItem(0).Click(driver);
                ProductConfigurePage productConfigurationPage = new ProductConfigurePage(driver);
                productConfigurationPage.ClickConfigurationService();
                productConfigurationPage.ClickNoneSelected();
                productConfigurationPage.EnterConfigurationServicesProjects(CFIProject);
                productConfigurationPage.ClickAddConfigurationProject();
                bool CommentsIsRequired = productConfigurationPage.Commentsrequired.IsElementDisplayed(driver, TimeSpan.FromSeconds(30));
                if (CommentsIsRequired)
                {
                    productConfigurationPage.Commentsrequired.SendKeys(CFIComment);
                }              
                bool SaveIsEnabled = productConfigurationPage.Saveoncomments.IsElementDisplayed(driver, TimeSpan.FromSeconds(30));
                if (SaveIsEnabled)
                {
                    productConfigurationPage.Saveoncomments.Click();
                }                
                //productConfigurationPage.Commentsrequired.Click();             
                //productConfigurationPage.Saveoncomments.Click();
                //productConfigurationPage.ClickSaveOnComments();
                productConfigurationPage.GoToCurrentQuotePage();
                new CreateQuotePage(driver).ReadyStockIQcheckbox.SelectCheckBox(driver);
                crQuotePage.RSIDNotification.IsElementDisplayed(driver).Equals(false);
                crQuotePage.BtnSaveQuote.IsElementClickable(driver).Equals(true);                
            }
            else
            {
                new CreateQuotePage(driver).ReadyStockIQcheckbox.SelectCheckBox(driver);
                crQuotePage.Notification.GetText(driver).Should().Contain("RSID cannot be created. Configuration is missing storage duration for Ready Stock. Please check project OEIS and if you have questions, contact your GCS contact.");
                crQuotePage.BtnSaveQuote.IsAttribtuePresent(driver, "disabled").Should().BeTrue();
