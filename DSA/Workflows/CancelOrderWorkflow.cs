...
namespace Dsa.Workflows
{
    ...
    public static class CancelOrderWorkflow
    {
        ...
        {
            var orderProcessingStatus = new OrderDetailsPage(driver).OrderProcessiongStatus.GetText(driver);
            var attempts = 10;
            ...
            while (attempts != 0)
            {
                ...
                driver.RefreshCurrentPage();
                driver.WaitFor(x => new OrderDetailsPage(driver).OrderProcessiongStatus.Displayed, 90);
                attempts -= 1;
