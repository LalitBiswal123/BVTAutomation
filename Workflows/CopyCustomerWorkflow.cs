    ...
    public class CopyCustomerWorkflow
    {
        ...
        public static string CopyCompanyFromCosumerToConsumerCompany(IWebDriver webDriver, DataRow row)
        {
            ...
            try
            {
                ...
                OrganizationSearchPage organizationSearchPage = HomeWorkflow.GoToOrganizationSearch(webDriver);
                organizationSearchPage.Validate();
                //Search by Customer Number 
                CustomerDetailsPage customerDetailsPage = organizationSearchPage.SearchPersonByCustomerNumber(row["CustomerNumber"].ToString());
                ...
                row["CompanyName"].ToString().Should().ShouldBeEquivalentTo(customerDetailsPage.DetailsHeader().Split(':')[0].Trim());
                ...
                row["CustomerName"].ToString().Should().ShouldBeEquivalentTo(customerDetailsPage.DetailsHeader().Split(':')[1].Trim());
                //Click More Action
                customerDetailsPage.MoreActionClk();
                customerDetailsPage.CopyCustomer1.Click();
        ...
        public static void VerfiyCustomerInformation(IWebDriver webDriver, string companyName, DataRow row)
        {
            try
            {
                ...
                row["CompanyName"].ToString().Should().ShouldBeEquivalentTo(customerDetailsPage.DetailsHeader().Split(':')[0].Trim());
                row["CompanyName"].ToString().Should().ShouldBeEquivalentTo(customerDetailsPage.TxtCompanyName.Text);
                ...
                row["CompanyNumber"].ToString().Should().ShouldBeEquivalentTo(customerDetailsPage.LblCompanyNumber.Text);
                //Verify Customer Class
                row["CustomerClass"].ToString().Should().ShouldBeEquivalentTo(customerDetailsPage.TxtCustomerClass.Text);
                //Verify Customer Category
                row["CustomerCategory"].ToString().Should().ShouldBeEquivalentTo(customerDetailsPage.TxtCustomerCategory.Text);
                //Verify Customer CFO Language
                row["ContactCFOLanguage"].ToString().Should().ShouldBeEquivalentTo(customerDetailsPage.TxtContactCFOLanguage.Text);
                //Verify Last Updated with Current date in MM/DD/YYYY
                DateTime.Today.ToString("MM/dd/yyyy").ShouldBeEquivalentTo(customerDetailsPage.TxtLastUpdated.Text);
                //Verify Status as Active
                "Active".ShouldBeEquivalentTo(customerDetailsPage.CustomerStatus.Text);
                //Verify Customer Hold as No
                "No".ShouldBeEquivalentTo(customerDetailsPage.CustomerHold.Text);
                //Verify Sales Rep ID:
                ...
                if (row["CompanyNumber"].ToString() != "19" || row["CompanyNumber"].ToString()!="29")// For Consumer Comapnies Billing Work Phone is Company Number
                    row["BillingWorkPhone"].ToString().Should().ShouldBeEquivalentTo(customerDetailsPage.TxtCompanyPhone.Text);
                else//For Commercial Companies need to Pass Company Phone seperately
                    row["CompanyPhone"].ToString().Should().ShouldBeEquivalentTo(customerDetailsPage.TxtCompanyPhone.Text);
                //Verify Company Fax with Fax
                row["BillingFax"].ToString().Should().ShouldBeEquivalentTo(customerDetailsPage.TxtCompanyFax.Text);
            }
            catch (Exception ex)
            {
